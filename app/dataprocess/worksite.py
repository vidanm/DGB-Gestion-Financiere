...
class Worksite(Categories):
    ...
    def calculate_year_expenses(self, month, year):
        ...
        return (
            exp["Débit"].astype(float).sum()
            - exp["Crédit"].astype(float).sum()
        )
    ...
    def calculate_bab(self, budMas, csvBab):
        ...
        return (
            self.__calculate_bois(budMas, csvBab),
    ...
    def __calculate_bois(self, budMas, csvBab):
        ...
        for poste in sp:
            surface_coffrante = (
                budMas[self.worksite_name + "-MQ"]
                .loc[budMas["SOUS-POSTE"] == poste]
                .sum()
            )

            metre_consomme_mois = csvBab.loc[
                (csvBab["TYPE"] == "M") & (csvBab["SOUS-POSTE"] == poste)
            ]["VALEUR"].sum()
            metre_consomme_cumul = csvBab.loc[
                (csvBab["TYPE"] == "C") & (csvBab["SOUS-POSTE"] == poste)
            ]["VALEUR"].sum()
            ratio_consommation = (
                metre_consomme_cumul / surface_coffrante
                if surface_coffrante != 0
                else 0
            )

            dep_cumul = (
                self.categories["BOIS"].loc[poste, "Dépenses cumulées"].sum()
            )
            print("\n\n\nDPENESES IJDQWILDJ : " + str(dep_cumul) + "\n\n\n")

            pumoyen = (
                dep_cumul / metre_consomme_cumul
                if metre_consomme_cumul != 0
                else 0
            )
            ratio_euro_metre = (
                dep_cumul / surface_coffrante if surface_coffrante != 0 else 0
            )
            tmp = pd.DataFrame(
                data=[
                    [
        ...
        out["Surface coffrante"] = (
            out["Surface coffrante"].astype(int).apply("{:0,.2f} m²".format)
        )
        out["M² consommé mois"] = (
            out["M² consommé mois"].astype(int).apply("{:0,.2f} m²".format)
        )
        out["M² consommé cumul"] = (
            out["M² consommé cumul"].astype(int).apply("{:0,.2f} m²".format)
        )
        out["Ratio consommation"] = (
            out["Ratio consommation"].astype(int).apply("{:0,.2f} m²".format)
        )
        out["PU Moyen"] = out["PU Moyen"].astype(int).apply("{:0,.2f}€".format)
        out["Ratio €/m²"] = (
            out["Ratio €/m²"].astype(int).apply("{:0,.2f}€".format)
        )
    ...
    def __calculate_aciers(self, budMas, csvBab):
        ...
        for poste in sp:
            type_acier = poste
            depenses_mois_kg = csvBab.loc[
                (csvBab["TYPE"] == "M") & (csvBab["SOUS-POSTE"] == poste)
            ]["VALEUR"].sum()
            depenses_cumul_kg = csvBab.loc[
                (csvBab["TYPE"] == "C") & (csvBab["SOUS-POSTE"] == poste)
            ]["VALEUR"].sum()
            reste_a_depenser = csvBab.loc[
                (csvBab["TYPE"] == "R") & (csvBab["SOUS-POSTE"] == poste)
            ]["VALEUR"].sum()

            dep_cumul = (
                self.categories["ACIERS"].loc[poste, "Dépenses cumulées"].sum()
            )
            budget_kg = (
                budMas[self.worksite_name + "-MQ"]
                .loc[budMas["SOUS-POSTE"] == poste]
            ...
            pu_moyen_etude = (
                budMas[self.worksite_name + "-AP"]
                .loc[budMas["SOUS-POSTE"] == poste]
                .sum()
            )
            pu_moyen_chantier = (
                dep_cumul / depenses_cumul_kg if depenses_cumul_kg != 0 else 0
            )
        ...
        out["Dépenses du mois"] = (
            out["Dépenses du mois"].astype(int).apply("{:0,.2f} kg".format)
        )
        out["Dépenses cumulées"] = (
            out["Dépenses cumulées"].astype(int).apply("{:0,.2f} kg".format)
        )
        ...
        out["PU Moyen etude"] = (
            out["PU Moyen etude"].astype(int).apply("{:0,.2f}€".format)
        )
        out["PU Moyen chantier"] = (
            out["PU Moyen chantier"].astype(int).apply("{:0,.2f}€".format)
        )
    ...
    def __calculate_beton(self, budMas, csvBab):
        ...
        for poste in sp:
            type_beton = poste
            quantite_du_mois = csvBab.loc[
                (csvBab["TYPE"] == "M") & (csvBab["SOUS-POSTE"] == poste)
            ]["VALEUR"].sum()
            quantite_cumul = csvBab.loc[
                (csvBab["TYPE"] == "C") & (csvBab["SOUS-POSTE"] == poste)
            ]["VALEUR"].sum()
            quantite_restante = csvBab.loc[
                (csvBab["TYPE"] == "R") & (csvBab["SOUS-POSTE"] == poste)
            ]["VALEUR"].sum()

            dep_cumul = (
                self.categories["BETON"].loc[poste, "Dépenses cumulées"].sum()
            )
            ...
            pu_moyen_chantier = (
                dep_cumul / quantite_cumul if quantite_cumul != 0 else 0
            )
        ...
        out["Quantité du mois"] = (
            out["Quantité du mois"].astype(int).apply("{:0,.2f} m³".format)
        )
        out["Quantité cumulée"] = (
            out["Quantité cumulée"].astype(int).apply("{:0,.2f} m³".format)
        )
        out["M³ Étude"] = (
            out["M³ Étude"].astype(int).apply("{:0,.2f} m³".format)
        )
        out["Quantité restante"] = (
            out["Quantité restante"].astype(int).apply("{:0,.2f} m³".format)
        )
            ...
            try:
                if row["POSTE"] not in not_used_rows:
                    self.categories[row["POSTE"]].loc[
                        row["SOUS-POSTE"], "Budget"
                    ] += round(row[self.worksite_name])
            except Exception:
                logging.write(
                    "Le couple "
                    + row["POSTE"]
    ...
    def add_marche_avenants(self, budMas):
        ...
        for poste in budMas["POSTE"].unique():
            ...
            for sp in tmp["SOUS-POSTE"]:
                self.categories[poste].loc[sp, "Marché"] = (
                    budMas[self.worksite_name + "-MQ"]
                    .loc[budMas["SOUS-POSTE"] == sp]
                    .sum()
                )
                self.categories[poste].loc[sp, "Avenants"] = (
                    budMas[self.worksite_name + "-AP"]
                    .loc[budMas["SOUS-POSTE"] == sp]
                    .sum()
                )

            self.categories[poste] = self.categories[poste][
                [
                    "Marché",
                    "Avenants",
                    "Dépenses du mois",
            ...
            self.categories[poste]["Marché"] = (
                self.categories[poste]["Marché"]
                .astype(int)
                .apply("{:0,.2f}€".format)
            )
            self.categories[poste]["Avenants"] = (
                self.categories[poste]["Avenants"]
                .astype(int)
                .apply("{:0,.2f}€".format)
            )
    ...
    def compose_pfdc_budget(self):
        ...
        for name in self.category_names:
            for _, row in self.categories[name].iterrows():
                ...
                self.categories[name].loc[row.name, "Ecart PFDC/Budget"] = (
                    row["Budget"] - pfdc
                )
    ...
    def add_category_total(self, name):
        ...
        for index, row in self.categories[name].iterrows():
            totalannee += self.categories[name].loc[
                row.name, "Dépenses cumulées"
            ]
            totalmois += self.categories[name].loc[
                row.name, "Dépenses du mois"
            ]
            ...
            totalecart += self.categories[name].loc[
                row.name, "Ecart PFDC/Budget"
            ]
    ...
    def calcul_divers_result(self, year):
        ...
        ca_cumul = Revenues(self.expenses.data).calculate_year_revenues(year)
        ...
        out["CA Cumulé"] = (
            out["CA Cumulé"].astype(int).apply("{:0,.2f}€".format)
        )

        out["Dépenses cumulées"] = (
            out["Dépenses cumulées"].astype(int).apply("{:0,.2f}€".format)
        )

        out["Marge brute"] = (
            out["Marge brute"].astype(int).apply("{:0,.2f}€".format)
        )
    ...
    def calcul_ges_prev(self):
        ...
        self.categories["GESPREV"] = self.categories["GESPREV"].sort_values(
            "Dépenses cumulées", ascending=False
        )
    ...
    def get_formatted_data(self, category_name):
        ...
        if category_name != "DIVERS":
            ...
            formatted["Ecart PFDC/Budget"] = formatted[
                "Ecart PFDC/Budget"
            ].apply("{:0,.2f}€".format)
        return formatted
