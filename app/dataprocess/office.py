    ...
    def _add_revenues(self, month, year):
        ...
        for name in self.category_names:
            for _, row in self.categories[name].iterrows():
                month_expenses = self.categories[name].loc[
    ...
    def add_category_total(self):
        ...
        for name in self.category_names:
            for index, row in self.categories[name].iterrows():
                month_total += self.categories[name].loc[
                    row.name, "Dépenses du mois"
                ]
                year_total += self.categories[name].loc[
                    row.name, "Dépenses cumulées"
                ]
                month_revenues += self.categories[name].loc[
                    row.name, "%CA MOIS"
                ]
                year_revenues += self.categories[name].loc[
                    row.name, "%CA Cumul"
                ]
