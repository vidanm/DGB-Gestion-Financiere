index_default_table = {"Dépenses du mois": "Dépenses du mois (B1)",
                 "Dépenses cumulées": "Dépenses cumulées (B)",
                 "Budget": "Budget (C)",
                 "RAD": "RAD (D)",
                 "PFDC": "PFDC (E = B + D)",
                 "Ecart PFDC/Budget": "Ecart PFDC/Budget (F = E - C)",
                 "CA": "CA (G)"}

index_marge_a_avancement = {
        "CA": "CA (G | G2, G3)",
        "Dépenses": "Dépense (B2, B3)",
        "Marge brute": "Marge brute (I = G - B | I2, I3)"}

index_marge_a_avancement_cumul = {
        "CA": "CA (G)",
        "Dépenses": "Dépense (B)",
        "Marge brute": "Marge brute (I = G - B)"}


index_marge_fdc = {"Marge brute": "Marge brute (J = V - E)"}

index_synthese = {
        "CHANTIER": "Chantier",
        "BUDGET": "Budget",
        "CA MOIS": "CA du mois (G1)",
        "DEP DU MOIS": "Dép. du mois (B1)",
        "MARGE MOIS" : "Marge du mois (I1 = G1 - B1)",
        "CA CUMUL" : "CA Cumulé (G) ",
        "DEP CUMULEES": "Dép. cumulées (B)",
        "MARGE A FIN DE MOIS": "Marge à l'av. (I3 = G - B)",
        "PFDC": "PFDC (E)",
        "MARGE FDC" : "Marge FDC (J)"}

index_synthese_annee = {
    "CHANTIER": "Chantier",
    "CA": "CA année (G2)",
    "Dépenses": "Dépenses année (B2)",
    "Reste à facturer": "Reste à fac. (W = V - G3)",
    "Marge €": "Marge année € (I2 = G2 - B2)",
    "Marge %": "Marge année % (I2 / G2)",
    "Marge fin annee €": "Marge fin d'année € (K = J - I2)",
    "Marge fin annee %": "Marge fin d'année % (K / W)"
    }
